...
from fla.ops.gated_delta_rule.backends.flashgdn.utils import prepare_chunk_indices
...
def tilelang_kkt_solve(
    ...
    def kernel_body(
        ...
        if tx < 128:
            ...
            # A = K @ K^T
            T.gemm_v1(k_shared, k_shared, a64_fragment, transpose_B=True, clear_accum=True)
            ...
            for k_r in T.unroll(16):
                for j_s, k_s, k_t in T.Parallel(2, 16, 16):
                    a16o_fragment[j_s, k_s, k_t] += a16i_shared[j_s * 2 + 1, k_s, k_r] * a16o_shared[j_s, k_r, k_t]
            for j_s, k_s, k_t in T.Parallel(2, 16, 16):
                a16o_shared[j_s, k_t, k_s] = a16o_fragment[j_s, k_s, k_t]
            T.clear(a16o_fragment)
            for k_r in T.unroll(16):
                for j_s, k_s, k_t in T.Parallel(2, 16, 16):
                    a16o_fragment[j_s, k_s, k_t] += a16o_shared[j_s, k_r, k_s] * a16i_shared[j_s * 2, k_r, k_t]
            ...
            for j_s, k_s, k_t in T.Parallel(2, 32, 32):
                if k_s // 16 == k_t // 16:
                    a32i_fragment[j_s, k_s, k_t] = a16i_shared[j_s * 2 + k_s // 16, k_s % 16, k_t % 16]
            for j_s, k_s, k_t in T.Parallel(2, 32, 32):
                if j_s == 0:
...
def kkt_solve(
    ...
    a = torch.empty((batch_size, num_tokens, H, chunk_size), dtype=k.dtype, device=k.device)
