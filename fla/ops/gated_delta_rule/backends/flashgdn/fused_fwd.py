...
from fla.ops.gated_delta_rule.backends.flashgdn.utils import prepare_chunk_offsets

MULTI_PROCESSOR_COUNT = torch.cuda.get_device_properties().multi_processor_count
TARGET_NUM_CTAS = int(MULTI_PROCESSOR_COUNT * 0.7)
LOG2_E = 1.4426950408889634
_FAST_MATH_ENABLED = os.environ.get("FLA_FLASH_GDN_DISABLE_FAST_MATH", "").lower() not in {
    "1",
    "true",
    "yes",
    "on",
...
def _tilelang_pass_configs() -> dict[tilelang.PassConfigKey, bool]:
    configs = {
        tilelang.PassConfigKey.TL_ENABLE_FAST_MATH: _FAST_MATH_ENABLED,
    }
    if _env_flag("FLA_FLASH_GDN_DISABLE_TMA_LOWER"):
        configs[tilelang.PassConfigKey.TL_DISABLE_TMA_LOWER] = True
    if _env_flag("FLA_FLASH_GDN_DISABLE_WARP_SPECIALIZED"):
        configs[tilelang.PassConfigKey.TL_DISABLE_WARP_SPECIALIZED] = True
    if _env_flag("FLA_FLASH_GDN_DISABLE_WGMMA"):
        configs[tilelang.PassConfigKey.TL_DISABLE_WGMMA] = True
    return configs


@tilelang.jit(
    # out_idx=[-3, -2, -1],
    pass_configs=_tilelang_pass_configs(),
)
            ...
            if tx < 128:
                ...
                for i_s in T.serial(num_iters):
                    ...
                    for j_s in T.Parallel(block_S):
                        g_exp_shared[j_s] = T.exp2(g_shared[i_s % 2, j_s] * LOG2_E)
                    for j_s in T.Parallel(block_S):
                        g_rev_exp_shared[j_s] = T.if_then_else(
                            seq_start_idx + i_s * block_S + j_s < seq_end_idx,
                            T.exp2((g_shared[i_s % 2, block_S - 1] - g_shared[i_s % 2, j_s]) * LOG2_E),
                            0.0,
                        )
                    T.barrier_arrive(bar_1)
                ...
                for i_s in T.serial(num_iters):
                    ...
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        if j_s >= j_t:
                            g_fragment[j_s, j_t] = T.exp2(g_fragment[j_s, j_t] * LOG2_E)
                        else:
                ...
                if tx < 384 + 32:
                    for i_s in T.serial(num_iters):
                        ...
                        # Load Q
                        T.copy(q[batch_idx, left:right, bhg, 0:DK], q_shared[i_s % 2, :, :])
                        # Load K
                        T.copy(k[batch_idx, left:right, bhg, 0:DK], k_shared[i_s % 2, :, :])
...
def fused_gdr_fwd(
    ...
    if raw_cu_seqlens is None:
        ...
        final_state = torch.empty((raw_cu_seqlens.shape[0] - 1, H, K, V), dtype=torch.float32, device=k.device)
