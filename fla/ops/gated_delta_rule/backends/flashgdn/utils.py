...
def tensor_cache(
    ...
    cache: OrderedDict[tuple[tuple[int, ...], tuple[tuple[str, int], ...]], tuple[tuple[Any, ...], dict[str, Any], Any]] = (
        OrderedDict()
    )
    cache_size = 256
...
def prepare_chunk_indices(
    ...
    indices = torch.cat([torch.arange(n) for n in tilelang.cdiv(prepare_lens(cu_seqlens), chunk_size).tolist()])
