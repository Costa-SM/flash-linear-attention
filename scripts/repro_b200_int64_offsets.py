...
def repro_conv() -> None:
    ...
    _report("causal_conv1d_fwd_kernel", got, ref)
...
def _wy_reference(
    ...
    for start in range(0, seq_len, block_t):
        ...
        kb[:, :length] = k[:, start:end].float() * (b_block * torch.exp(g[:, start:end].float()))[..., None]
...
def repro_norm() -> None:
    ...
    torch.cuda.synchronize()
    _report("layer_norm_gated_fwd_kernel", got, ref)
